...
const tutorController = {
    ...
    addTutorRequest: async (req, res) => {
      try {
          const { classId, sessionId, reason,type, shift, newSession,oldShiftId } = req.body;
          const user = req.user;
          ...
          switch (type) {
              case 'shift_reschedule':
                  if (!shift || shift.dayOfWeek==null  || !shift.startTime || !shift.endTime) {
                      return res.status(400).json({ status: "error", message: "Complete shift details are required" });
                  }
                  ...
                  if (oldShiftId) {
                      ...
                      if (!oldShift) {
                          return res.status(400).json({ status: "error", message: "Old shift not found" });
                      }
                      requestData.oldShiftId = oldShiftId;
                      if(oldShift.dayOfWeek != shift.dayOfWeek){
                        return res.status(400).json({ status: "error", message: "Please request the change for same day." });
                      }
              ...
              case 'session_cancel':
                  ...
                  requestData.message = "Class cancellation request for " + session.date.toDateString();
              ...
              switch (request.type) {
                  case 'shift_reschedule':
                      ...
                      if (request.oldShiftId) {
                          // Remove old shift
                          tutor.shifts = tutor.shifts.filter(shift => 
                              shift._id.toString() !== request.oldShiftId
                          );
                      }
                      // Add new shift
